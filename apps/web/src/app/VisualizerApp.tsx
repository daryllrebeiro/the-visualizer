...
import { type DomainKey, DOMAIN_OPTIONS } from './domain-options';
...
import {
  ...
  type VectorDBClusterState,
  VectorDBInvariantChecker,
  type VectorDBSimEvent,
...
export default function VisualizerApp({
  initialDomain = 'kafka',
}: VisualizerAppProps): React.JSX.Element {
  const [restUrl, setRestUrl] = useState(getDefaultRestUrl);
  ...
  const [showCompositePipelinesModal, setShowCompositePipelinesModal] = useState(false);
  const [shareToastMessage, setShareToastMessage] = useState<string | null>(null);
  const [perfMetrics, setPerfMetrics] = useState<RenderPerfMetrics>({
    fps: 60,
    frameTimeMs: 1.2,
    renderedEntities: 0,
    culledEntities: 0,
  ...
  const llmServingRngRef = useRef<DeterministicRNG>(new DeterministicRNG(42));
  const llmServingInvariantCheckerRef = useRef<LLMServingInvariantChecker>(
    new LLMServingInvariantChecker(),
  );

  const [vectorDbState, setVectorDbState] = useState<VectorDBClusterState>(() =>
    createDefaultVectorDBCluster(),
  );
  const vectorDbRngRef = useRef<DeterministicRNG>(new DeterministicRNG(42));
  const vectorDbInvariantCheckerRef = useRef<VectorDBInvariantChecker>(
    new VectorDBInvariantChecker(),
  );

  const [gpuClusterState, setGpuClusterState] = useState<GPUClusterState>(() =>
    createDefaultGPUCluster(),
  );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setLlmServingState((prev) => {
        const ev: LLMServingSimEvent = {
          id: `llm-tick-${String(prev.tick + 1)}`,
          tick: prev.tick + 1,
          type: 'LLM_TICK',
          payload: {},
        };
        const res = pureLLMServingTransition(prev, ev, llmServingRngRef.current);
        const violation = llmServingInvariantCheckerRef.current.check(res.nextState);
        if (violation && !violation.isPedagogicalFlaw) {
          setIsHalted(true);
          setHaltError(`[LLM Serving ${violation.ruleId}] ${violation.description}`);
        }
        return res.nextState;
      });
    }, 500);
  ...
  // Vector DB Simulation Loop (HNSW Graph / Quantization maintenance)
  useEffect(() => {
    if (selectedDomain !== 'vectordb' || isPaused) return;
    const interval = setInterval(() => {
      setVectorDbState((prev) => {
        const ev: VectorDBSimEvent = {
          id: `vdb-tick-${String(prev.tick + 1)}`,
          tick: prev.tick + 1,
          type: 'VEC_TICK',
          payload: {},
        };
        const res = pureVectorDBTransition(prev, ev, vectorDbRngRef.current);
        const violation = vectorDbInvariantCheckerRef.current.check(res.nextState);
        if (violation && !violation.isPedagogicalFlaw) {
          setIsHalted(true);
          setHaltError(`[VectorDB ${violation.ruleId}] ${violation.description}`);
        }
        return res.nextState;
      });
    }, 1000);
  ...
  // GPU Cluster Simulation Loop (1F1B Schedule / Ring-AllReduce / ZeRO)
  useEffect(() => {
    if (selectedDomain !== 'gpu-cluster' || isPaused) return;
    const interval = setInterval(() => {
      setGpuClusterState((prev) => {
        const ev: GPUClusterSimEvent = {
          id: `gpu-tick-${String(prev.tick + 1)}`,
          tick: prev.tick + 1,
          type: 'GPU_TICK',
          payload: {},
        };
        const res = pureGPUClusterTransition(prev, ev, gpuClusterRngRef.current);
        const violation = gpuClusterInvariantCheckerRef.current.check(res.nextState);
        if (violation && !violation.isPedagogicalFlaw) {
          setIsHalted(true);
          setHaltError(`[GPU Cluster ${violation.ruleId}] ${violation.description}`);
        }
        return res.nextState;
      });
    }, 500);
  ...
  const handleResetSimulation = (): void => {
    ...
    vectorDbRngRef.current = new DeterministicRNG(42);
    setGpuClusterState(createDefaultGPUCluster());
    gpuClusterRngRef.current = new DeterministicRNG(42);
    addLog(
  ...
  // Auto-hydrate domain/state from URL permalink on mount
  useEffect(() => {
    ...
    if (decoded) {
      if (decoded.domain && DOMAIN_OPTIONS.some((d) => d.id === decoded.domain)) {
        setSelectedDomain(decoded.domain as DomainKey);
      }
      if (decoded.tick !== undefined) {
        setPlaybackTick(decoded.tick);
      }
      addLog(
        `[Permalink] Hydrated shared session: ${decoded.domain ?? 'cluster'} at tick ${decoded.tick ?? 0}`,
        'SUCCESS',
      );
    }
  }, []);
  ...
  const handleSharePermalink = async (): Promise<void> => {
    ...
    else if (selectedDomain === 'raft') currentTick = raftState.tick;
    else if (selectedDomain === 'database') currentTick = dbState.tick;
    else if (selectedDomain === 'redis') currentTick = redisState.tick;
    else if (selectedDomain === 'kubernetes') currentTick = k8sState.tick;
    else if (selectedDomain === 'rabbitmq') currentTick = rabbitState.tick;
    else if (selectedDomain === 'storage') currentTick = storageState.tick;
    else if (selectedDomain === 'networking') currentTick = networkingState.tick;
    else if (selectedDomain === 'rate-limiter') currentTick = rateLimiterState.tick;
    else if (selectedDomain === 'distributed-lock') currentTick = distLockState.tick;
    ...
    const ok = await copyPermalinkToClipboard({
      domain: selectedDomain,
      tick: currentTick,
    });
    if (ok) {
      setShareToastMessage(`Copied permalink for ${selectedDomain.toUpperCase()}!`);
      addLog(`[Share] Permalink copied to clipboard for ${selectedDomain}`, 'SUCCESS');
      setTimeout(() => setShareToastMessage(null), 3000);
    } else {
      addLog(`[Share] Failed to copy link to clipboard`, 'WARN');
    }
  };

  const handleRunDomainScenario = (scenarioId: string, targetDomain?: DomainKey): void => {
    ...
    if (dom === 'kafka') {
      ...
      addLog(`[Drill Launched] Executing scenario "${scenarioId}" on ${dom.toUpperCase()}`, 'INFO');
      if (dom === 'rate-limiter') {
        handleRateLimiterBurst('client-1', 15);
      } else if (dom === 'distributed-lock') {
        handleDistLockInjectGcPause('client-1', 10);
      } else if (dom === 'id-gen') {
        ...
        handleRaftProposeCommand('replicate-log-entry');
      } else if (dom === 'redis') {
        handleRedisCrashNode('1');
      } else if (dom === 'kubernetes') {
        handleK8sScaleDeployment('api-gateway', 6);
      } else if (dom === 'rabbitmq') {
        handleRabbitPublish('orders-direct', 'order.created', 'Order #9999', null);
      } else if (dom === 'storage') {
        ...
        handleVectorDbInsertVector('v-drill', [0.8, 0.2, 0.4, 0.9]);
      } else if (dom === 'gpu-cluster') {
        handleGpuClusterStep1F1B();
      }
    }
  };
  ...
  const handleLlmSubmitRequest = (
    ...
    setLlmServingState((prev) => {
      const ev: LLMServingSimEvent = {
        id: `llm-req-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'LLM_SUBMIT_REQUEST',
        payload: { requestId, promptTokens, maxGeneratedTokens },
      };
      const res = pureLLMServingTransition(prev, ev, llmServingRngRef.current);
      addLog(
        `[LLM Serving] Submitted Request ${requestId} (Prompt: ${promptTokens}, Max: ${maxGeneratedTokens})`,
        'INFO',
      );
      return res.nextState;
    });
  ...
  const handleLlmStepBatch = (): void => {
    setLlmServingState((prev) => {
      const ev: LLMServingSimEvent = {
        id: `llm-step-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'LLM_STEP_BATCH',
        payload: {},
      };
  ...
  const handleLlmToggleSpeculative = (enabled: boolean, gamma?: number): void => {
    setLlmServingState((prev) => {
      const ev: LLMServingSimEvent = {
        id: `llm-spec-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'LLM_TOGGLE_SPECULATIVE',
        payload: { enabled, gamma },
      };
      const res = pureLLMServingTransition(prev, ev, llmServingRngRef.current);
      addLog(
        `[LLM Serving] Speculative Decoding ${enabled ? `Enabled (γ=${String(gamma ?? 3)})` : 'Disabled'}`,
        'INFO',
      );
      return res.nextState;
    });
  ...
  const handleLlmPreemptRequest = (requestId: string): void => {
    setLlmServingState((prev) => {
      const ev: LLMServingSimEvent = {
        id: `llm-preempt-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'LLM_PREEMPT_REQUEST',
        payload: { requestId },
      };
      const res = pureLLMServingTransition(prev, ev, llmServingRngRef.current);
      addLog(`[LLM Serving Chaos] Preempted & Evicted Request ${requestId}`, 'WARN');
      return res.nextState;
    });
  };

  const handleLlmInjectOOM = (count: number): void => {
    setLlmServingState((prev) => {
      const ev: LLMServingSimEvent = {
        id: `llm-oom-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'LLM_OOM_INJECTION',
        payload: { count },
      };
      const res = pureLLMServingTransition(prev, ev, llmServingRngRef.current);
      addLog(`[LLM Serving Chaos] Injected OOM Storm: ${count} concurrent requests`, 'WARN');
      return res.nextState;
    });
  };

  // VectorDB Handlers
  const handleVectorDbInsertVector = (
    nodeId: string,
    vector: number[],
    topLayer?: number,
  ): void => {
    setVectorDbState((prev) => {
      const ev: VectorDBSimEvent = {
        id: `vdb-ins-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'VEC_INSERT_VECTOR',
        payload: { nodeId, vector, topLayer },
      };
      const res = pureVectorDBTransition(prev, ev, vectorDbRngRef.current);
      addLog(`[VectorDB] Inserted Vector ${nodeId} at Top Layer ${String(topLayer ?? 0)}`, 'INFO');
      return res.nextState;
    });
  };

  const handleVectorDbQueryKNN = (queryId: string, queryVector: number[], k?: number): void => {
    setVectorDbState((prev) => {
      const ev: VectorDBSimEvent = {
        id: `vdb-query-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'VEC_QUERY_KNN',
        payload: { queryId, queryVector, k },
      };
      const res = pureVectorDBTransition(prev, ev, vectorDbRngRef.current);
      addLog(`[VectorDB] Queried k-NN (k=${String(k ?? 3)})`, 'INFO');
      return res.nextState;
    });
  };

  const handleVectorDbToggleIndexType = (indexType: 'HNSW' | 'IVF_PQ'): void => {
    setVectorDbState((prev) => {
      const ev: VectorDBSimEvent = {
        id: `vdb-idx-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'VEC_TOGGLE_INDEX_TYPE',
        payload: { indexType },
      };
      const res = pureVectorDBTransition(prev, ev, vectorDbRngRef.current);
      addLog(`[VectorDB] Switched Index Paradigm to ${indexType}`, 'INFO');
      return res.nextState;
    });
  };

  // GPU Cluster Handlers
  const handleGpuClusterStep1F1B = (): void => {
    setGpuClusterState((prev) => {
      const ev: GPUClusterSimEvent = {
        id: `gpu-1f1b-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'GPU_STEP_1F1B',
        payload: {},
      };
  ...
  const handleGpuClusterStepAllReduce = (): void => {
    setGpuClusterState((prev) => {
      const ev: GPUClusterSimEvent = {
        id: `gpu-ar-${String(Date.now())}`,
        tick: prev.tick + 1,
  ...
  const handleGpuClusterSetZeroStage = (stage: any): void => {
    setGpuClusterState((prev) => {
      const ev: GPUClusterSimEvent = {
        id: `gpu-zero-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'GPU_SET_ZERO_STAGE',
        payload: { stage },
      };
      const res = pureGPUClusterTransition(prev, ev, gpuClusterRngRef.current);
      addLog(`[GPU Cluster] Switched ZeRO Stage to ${String(stage)}`, 'INFO');
      return res.nextState;
    });
  };

  const handleGpuClusterThrottleStraggler = (gpuId: string, throttled: boolean): void => {
    setGpuClusterState((prev) => {
      const ev: GPUClusterSimEvent = {
        id: `gpu-strag-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'GPU_THROTTLE_STRAGGLER',
        payload: { gpuId, throttled },
      };
      const res = pureGPUClusterTransition(prev, ev, gpuClusterRngRef.current);
      addLog(`[GPU Cluster Chaos] Straggler throttle on ${gpuId}: ${String(throttled)}`, 'WARN');
      return res.nextState;
    });
  };

  const handleGpuClusterSeverNVLink = (sourceGPU: string, targetGPU: string): void => {
    setGpuClusterState((prev) => {
      const ev: GPUClusterSimEvent = {
        id: `gpu-link-${String(Date.now())}`,
        tick: prev.tick + 1,
        type: 'GPU_SEVER_NVLINK',
        payload: { sourceGPU, targetGPU },
      };
      const res = pureGPUClusterTransition(prev, ev, gpuClusterRngRef.current);
  ...
  const accessibleRows = React.useMemo(() => {
    ...
    if (selectedDomain === 'llm-serving') {
      ...
      return [
        {
          id: 'llm-engine',
          name: 'PagedAttention Engine (vLLM/Orca)',
          roleOrType: 'Continuous Batching Scheduler',
          status: 'ACTIVE',
          metrics: `Running: ${runningCount} | Waiting: ${waitingCount} | Free Blocks: ${llmServingState.kvBlockPool.freeBlockIndices.length}/${llmServingState.kvBlockPool.totalBlocks}`,
        },
      ];
    }
    if (selectedDomain === 'vectordb') {
      return Object.values(vectorDbState.hnswGraph.nodes).map((n) => ({
        id: `vdb-node-${n.id}`,
        name: `HNSW Node ${n.id}`,
        roleOrType: `Top Layer ${n.topLayer}`,
        status: 'INDEXED',
        metrics: `Neighbors: L0=${n.neighborsByLayer[0]?.length ?? 0}, L1=${n.neighborsByLayer[1]?.length ?? 0}, L2=${n.neighborsByLayer[2]?.length ?? 0}`,
      }));
    }
    if (selectedDomain === 'gpu-cluster') {
      return Object.values(gpuClusterState.gpus).map((g) => ({
        id: `gpu-${g.id}`,
        name: `GPU #${g.id} (${g.name})`,
        roleOrType: `Rack ${g.rackId}`,
        status: g.status,
  ...
  return (
    <div className="app-shell">
        ...
        <div className="header-right">
          ...
          <div className="header-actions">
            <button
              ...
              title="Search all domains, scenarios & controls (Cmd+K / Ctrl+K)"
              style={{ display: 'flex', alignItems: 'center', gap: '5px' }}
            >
              <kbd style={{ fontSize: '0.7rem', padding: '1px 5px', backgroundColor: '#334155', borderRadius: '4px', color: '#cbd5e1' }}>⌘K</kbd>
              <span>Search</span>
            </button>
            ...
            <button
              ...
              title="Open System Design Interview Prep mode with interactive drills"
            >
              <span>🎓 Interview Prep</span>
            </button>

            <button
              onClick={() => setShowCompositePipelinesModal(true)}
              className="btn btn--indigo"
              style={{
              ...
              title="Open Multi-Domain Composite System Pipelines"
            >
              <span>🔀 Pipelines</span>
            </button>
            ...
            <button
              ...
              className={`btn ${showFpsMonitor ? 'btn--primary' : 'btn--secondary'}`}
            ...
            <button
              ...
              className="btn btn--secondary"
              title="Generate shareable URL permalink for this scenario and state"
            >
              🔗 Share
            </button>
      ...
      <CommandPaletteModal
        ...
        onSelectDomain={(dom) => setSelectedDomain(dom)}
        onRunScenario={handleRunDomainScenario}
        onTogglePause={() => setIsPaused((prev) => !prev)}
        onReset={handleResetSimulation}
        onOpenInterviewPrep={() => setShowInterviewModal(true)}
        onOpenCompositePipelines={() => setShowCompositePipelinesModal(true)}
      ...
      <InterviewPrepModal
        ...
        onLaunchDrill={(domain, scenarioId) => handleRunDomainScenario(scenarioId, domain)}
      />

      {/* ── Composite Multi-Domain Pipelines Mode ── */}
      <CompositePipelineModal
        isOpen={showCompositePipelinesModal}
        onClose={() => setShowCompositePipelinesModal(false)}
        onSelectDomain={(dom) => setSelectedDomain(dom)}
        onRunStageDrill={(dom, sc) => handleRunDomainScenario(sc ?? 'drill', dom)}
      />
